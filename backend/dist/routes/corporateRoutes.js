...
exports.corporateRouter = void 0;
const express_1 = require("express");
const db_1 = require("../db");
const authMiddleware_1 = require("../middlewares/authMiddleware");
exports.corporateRouter = (0, express_1.Router)();
/**
 * @route POST /api/corporate/register
 * @desc Register a new corporate account
 */
exports.corporateRouter.post('/register', authMiddleware_1.authenticateJWT, async (req, res) => {
    ...
    try {
        await db_1.db.transaction(async (client) => {
            ...
            const corpRes = await client.query(`INSERT INTO corporate_accounts (company_name, tin_number, billing_address) 
         VALUES ($1, $2, $3) RETURNING *`, [companyName, tinNumber || '', billingAddress || '']);
            ...
            await client.query(`INSERT INTO corporate_employees (corporate_id, user_id, role) 
         VALUES ($1, $2, 'manager')`, [corporate.id, userId]);
            res.status(201).json({
 ...
 */
exports.corporateRouter.post('/employees', authMiddleware_1.authenticateJWT, async (req, res) => {
    const { employeeEmail, employeePhone, employeeName, role } = req.body;
    const userId = req.user?.id;
    try {
        // 1. Check if caller is a corporate manager
        const managerCheck = await db_1.db.query(`SELECT corporate_id FROM corporate_employees WHERE user_id = $1 AND role = 'manager'`, [userId]);
        ...
        let employeeUserRes = await db_1.db.query('SELECT id FROM users WHERE phone_number = $1', [employeePhone]);
        let employeeUserId;
        if (employeeUserRes.rowCount === 0) {
            const newUser = await db_1.db.query(`INSERT INTO users (phone_number, password_hash, role, full_name) 
         VALUES ($1, $2, 'customer', $3) RETURNING id`, [employeePhone, passwordHash, employeeName]);
        ...
        await db_1.db.query(`INSERT INTO corporate_employees (corporate_id, user_id, role) 
       VALUES ($1, $2, $3) 
       ON CONFLICT (user_id) DO UPDATE SET role = EXCLUDED.role`, [corporateId, employeeUserId, role || 'employee']);
 ...
 */
exports.corporateRouter.get('/bookings', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    try {
        const employeeCheck = await db_1.db.query(`SELECT corporate_id, role FROM corporate_employees WHERE user_id = $1`, [userId]);
        ...
        const bookingsRes = await db_1.db.query(query, params);
 ...
 */
exports.corporateRouter.get('/invoices', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    try {
        const employeeCheck = await db_1.db.query(`SELECT corporate_id FROM corporate_employees WHERE user_id = $1 AND role = 'manager'`, [userId]);
        if (employeeCheck.rowCount === 0) {
        ...
        const invoicesRes = await db_1.db.query(`SELECT b.id as booking_id, b.date, b.address, b.description, b.service_price, 
              u.full_name as employee_name, f.full_name as fundi_name, b.status
       FROM bookings b
       JOIN users u ON b.customer_id = u.id
       JOIN users f ON b.fundi_id = f.id
       WHERE b.corporate_id = $1 AND b.status = 'completed'`, [corporateId]);
 ...
 */
exports.corporateRouter.post('/approve-expense', authMiddleware_1.authenticateJWT, async (req, res) => {
