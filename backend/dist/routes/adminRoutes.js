 ...
 * Audit log helper
 */
async function writeAuditLog(adminId, action, ip, userAgent, details) {
    try {
        await db_1.db.query(`INSERT INTO audit_logs (user_id, action, ip_address, user_agent, details)
       VALUES ($1, $2, $3, $4, $5)`, [adminId, action, ip, userAgent, JSON.stringify(details)]);
    }
    catch (err) {
 ...
 */
exports.adminRouter.get('/audit-logs', requireRoles(['super_admin']), async (req, res) => {
    try {
        const logs = await db_1.db.query(`SELECT al.id, al.action, al.ip_address, al.user_agent, al.details, al.created_at,
 ...
 */
exports.adminRouter.post('/accounts', requireRoles(['super_admin']), async (req, res) => {
    ...
    try {
        ...
        const newAdmin = await db_1.db.query(`INSERT INTO users (full_name, email, phone_number, password_hash, role, referral_code, email_verified, phone_verified)
       VALUES ($1, $2, $3, $4, $5, $6, true, true) RETURNING id, full_name, email, role`, [fullName, email, phoneNumber, passwordHash, role, referralCode]);
        await writeAuditLog(admin.id, 'create_admin_account', req.ip || '127.0.0.1', req.headers['user-agent'] || '', { newAdminId: newAdmin.rows[0].id, role });
 ...
 */
exports.adminRouter.delete('/accounts/:id', requireRoles(['super_admin']), async (req, res) => {
    const { id } = req.params;
    const admin = req.body.adminUser;
    try {
        await db_1.db.query('UPDATE users SET status = $1 WHERE id = $2', ['banned', id]);
 ...
 */
exports.adminRouter.get('/fundis/pending', requireRoles(['verification_officer', 'admin', 'super_admin']), async (req, res) => {
    try {
        const list = await db_1.db.query(`SELECT u.id, u.full_name, u.email, u.phone_number, u.profile_picture_url,
 ...
 */
exports.adminRouter.post('/fundis/:id/verify', requireRoles(['verification_officer', 'admin', 'super_admin']), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        await db_1.db.query(`UPDATE fundi_profiles 
       SET verification_status = $1, verified_badge = $2, verification_notes = $3
       WHERE user_id = $4`, [status, badge, notes || '', id]);
 ...
 */
exports.adminRouter.post('/disputes/:id/resolve', requireRoles(['finance_officer', 'admin', 'super_admin']), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const disputeRes = await db_1.db.query('SELECT * FROM disputes WHERE id = $1 FOR UPDATE', [id]);
        ...
        await db_1.db.transaction(async (client) => {
            if (winner === 'customer') {
                // Refund customer
                await walletService_1.WalletService.refundEscrow(dispute.booking_id);
            }
            else {
                // Release to fundi
                await walletService_1.WalletService.releaseEscrow(dispute.booking_id);
            }
            // Close dispute
         ...
         WHERE id = $3`, [winner === 'customer' ? 'resolved_refunded' : 'resolved_released', admin.id, id]);
        });
 ...
 */
exports.adminRouter.post('/settings', requireRoles(['super_admin']), async (req, res) => {
    const { commissionPercent, vatPercent } = req.body;
    const admin = req.body.adminUser;
    try {
        if (commissionPercent !== undefined) {
            await db_1.db.query("UPDATE system_settings SET value = $1 WHERE key = 'platform_commission_percent'", [commissionPercent.toString()]);
        }
        if (vatPercent !== undefined) {
            await db_1.db.query("UPDATE system_settings SET value = $1 WHERE key = 'platform_vat_percent'", [vatPercent.toString()]);
        }
 ...
 */
exports.adminRouter.post('/app/release', requireRoles(['super_admin']), async (req, res) => {
    ...
    try {
        ...
        const releaseRes = await db_1.db.query(`INSERT INTO app_releases (version_code, type, download_url, release_notes, force_update)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`, [versionCode, type, downloadUrl, releaseNotes || '', forceUpdate || false]);
 ...
 */
exports.adminRouter.get('/app/releases', requireRoles(['admin', 'super_admin']), async (req, res) => {
    try {
        const releases = await db_1.db.query(`SELECT * FROM app_releases ORDER BY created_at DESC`);
 ...
 */
exports.adminRouter.patch('/app/releases/:id', requireRoles(['super_admin']), async (req, res) => {
    const { id } = req.params;
    const { isPublished, forceUpdate, releaseNotes } = req.body;
    const admin = req.body.adminUser;
    try {
        const updateRes = await db_1.db.query(`UPDATE app_releases 
       SET is_published = COALESCE($1, is_published),
           force_update = COALESCE($2, force_update),
           release_notes = COALESCE($3, release_notes)
       WHERE id = $4 RETURNING *`, [isPublished, forceUpdate, releaseNotes, id]);
 ...
 */
exports.adminRouter.get('/app/analytics/summary', requireRoles(['admin', 'super_admin']), async (req, res) => {
    try {
        const dailyAnalytics = await db_1.db.query(`SELECT platform, COUNT(*)::int as count, TO_CHAR(created_at, 'YYYY-MM-DD') as day 
       FROM app_download_analytics 
       GROUP BY platform, day 
       ORDER BY day DESC LIMIT 30`);
        const monthlyAnalytics = await db_1.db.query(`SELECT platform, COUNT(*)::int as count, TO_CHAR(created_at, 'YYYY-MM') as month 
       FROM app_download_analytics 
       GROUP BY platform, month 
       ORDER BY month DESC LIMIT 12`);
        const platformTotals = await db_1.db.query(`SELECT platform, COUNT(*)::int as count 
       FROM app_download_analytics 
       GROUP BY platform`);
        res.status(200).json({
 ...
 */
exports.adminRouter.post('/professions', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.db.query(`INSERT INTO professions (name_en, name_sw, icon_name)
       VALUES ($1, $2, $3) RETURNING *`, [nameEn, nameSw, iconName]);
 ...
 */
exports.adminRouter.put('/professions/:id', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { id } = req.params;
    const { nameEn, nameSw, iconName, isActive } = req.body;
    const admin = req.body.adminUser;
    try {
        const result = await db_1.db.query(`UPDATE professions 
       SET name_en = COALESCE($1, name_en),
           name_sw = COALESCE($2, name_sw),
           icon_name = COALESCE($3, icon_name),
           is_active = COALESCE($4, is_active)
       WHERE id = $5 RETURNING *`, [nameEn, nameSw, iconName, isActive, id]);
 ...
 */
exports.adminRouter.delete('/professions/:id', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { id } = req.params;
    const admin = req.body.adminUser;
    try {
        // We do a soft delete by deactivating it
        const result = await db_1.db.query(`UPDATE professions SET is_active = false WHERE id = $1 RETURNING *`, [id]);
        if (result.rowCount === 0) {
            res.status(404).json({ error: 'Profession category not found.' });
            return;
        }
 ...
 */
exports.adminRouter.patch('/users/:id/status', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const result = await db_1.db.query(`UPDATE users SET status = $1 WHERE id = $2 RETURNING id, full_name, status`, [status, id]);
        if (result.rowCount === 0) {
 ...
 */
exports.adminRouter.get('/verification-queue', requireRoles(['admin', 'super_admin', 'verification_officer']), async (req, res) => {
    try {
        const queue = await db_1.db.query(`SELECT 
        u.id as user_id,
        u.full_name,
        u.phone_number,
 ...
 */
exports.adminRouter.patch('/fundi/:fundiId/primary-profession', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
        ...
        await db_1.db.transaction(async (client) => {
            ...
            await client.query(`UPDATE fundi_professions SET "isPrimary" = false WHERE "fundiId" = $1`, [fundiId]);
            // 2. Set the requested profession to isPrimary = true
            const result = await client.query(`UPDATE fundi_professions SET "isPrimary" = true WHERE "fundiId" = $1 AND "professionId" = $2 RETURNING *`, [fundiId, primaryProfessionId]);
            // If the profession wasn't already mapped, we insert it
            if (result.rowCount === 0) {
                await client.query(`INSERT INTO fundi_professions (id, "fundiId", "professionId", "isPrimary") 
           VALUES (gen_random_uuid(), $1, $2, true)`, [fundiId, primaryProfessionId]);
            }
        });
 ...
 */
exports.adminRouter.patch('/fundi/:fundiId/approve-secondary', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { fundiId } = req.params;
    ...
    try {
        ...
        await db_1.db.transaction(async (client) => {
            ...
            await client.query(`DELETE FROM fundi_professions WHERE "fundiId" = $1 AND "isPrimary" = false`, [fundiId]);
            // 2. Insert new secondary professions
            for (const profId of secondaryProfessionIds) {
                // Prevent inserting same as primary
                const checkPrim = await client.query(`SELECT 1 FROM fundi_professions WHERE "fundiId" = $1 AND "professionId" = $2 AND "isPrimary" = true`, [fundiId, profId]);
                if (checkPrim.rowCount === 0) {
                    await client.query(`INSERT INTO fundi_professions (id, "fundiId", "professionId", "isPrimary") 
             VALUES (gen_random_uuid(), $1, $2, false)`, [fundiId, profId]);
                }
            }
        });
 ...
 */
exports.adminRouter.post('/professions/merge', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
        ...
        await db_1.db.transaction(async (client) => {
            ...
            const sourceFundis = await client.query(`SELECT "fundiId", "isPrimary" FROM fundi_professions WHERE "professionId" = $1`, [sourceId]);
            for (const row of sourceFundis.rows) {
                const { fundiId, isPrimary } = row;
                // Check if fundi already has target profession
                const checkTarget = await client.query(`SELECT id, "isPrimary" FROM fundi_professions WHERE "fundiId" = $1 AND "professionId" = $2`, [fundiId, targetId]);
                if (checkTarget.rowCount === 0) {
                    // Fundi doesn't have target profession: update source record to target profession
                    await client.query(`UPDATE fundi_professions SET "professionId" = $1 WHERE "fundiId" = $2 AND "professionId" = $3`, [targetId, fundiId, sourceId]);
                }
                else {
                    ...
                    if (isPrimary && !checkTarget.rows[0].isPrimary) {
                        await client.query(`UPDATE fundi_professions SET "isPrimary" = true WHERE id = $1`, [checkTarget.rows[0].id]);
                    }
                    // Delete duplicate source record
                    await client.query(`DELETE FROM fundi_professions WHERE "fundiId" = $1 AND "professionId" = $2`, [fundiId, sourceId]);
                }
            }
            // 2. Move services pointing to source category to target category
            await client.query(`UPDATE services SET "categoryId" = $1 WHERE "categoryId" = $2`, [targetId, sourceId]);
            // 3. Deactivate source category
            await client.query(`UPDATE professions SET is_active = false WHERE id = $1`, [sourceId]);
        });
 ...
 */
exports.adminRouter.patch('/verify/certificate/:id', requireRoles(['admin', 'super_admin', 'verification_officer']), async (req, res) => {
    const { id } = req.params;
    const { status, notes } = req.body; // status: 'verified' or 'rejected'
    const admin = req.body.adminUser;
    try {
        ...
        const certRes = await db_1.db.query(`UPDATE certificates 
 ...
 */
exports.adminRouter.patch('/verify/license/:id', requireRoles(['admin', 'super_admin', 'verification_officer']), async (req, res) => {
    const { id } = req.params;
    const { status, notes } = req.body; // status: 'verified' or 'rejected'
    const admin = req.body.adminUser;
    try {
        ...
        const licRes = await db_1.db.query(`UPDATE licenses 
       SET status = $1 
       WHERE id = $2 RETURNING *`, [licStatus, id]);
        ...
        if (status === 'verified') {
         ...
         WHERE user_id = $1`, [license.fundi_profile_id]);
        }
        // Write audit log
 ...
 */
exports.adminRouter.patch('/verify/badge', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.db.query(query, [value === true || value === 'true', fundiId]);
        if (result.rowCount === 0) {
 ...
 */
exports.adminRouter.post('/verify/request-docs', requireRoles(['admin', 'super_admin', 'verification_officer']), async (req, res) => {
    ...
    try {
       ...
       WHERE user_id = $2`, [message, fundiId]);
 ...
 */
exports.adminRouter.get('/config', requireRoles(['admin', 'super_admin']), async (req, res) => {
    try {
        const configRes = await db_1.db.query('SELECT * FROM system_configs LIMIT 1');
 ...
 */
exports.adminRouter.post('/config', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { commissionRate, subBasic, subPro, subPremium, promotionFee, inspectionFeeBase, corporateDiscount } = req.body;
    const admin = req.body.adminUser;
    try {
        const configRes = await db_1.db.query(`UPDATE system_configs 
       SET commission_rate = COALESCE($1, commission_rate),
           ...
           updated_at = NOW()
       RETURNING *`, [commissionRate, subBasic, subPro, subPremium, promotionFee, inspectionFeeBase, corporateDiscount]);
        if (admin) {
 ...
 */
exports.adminRouter.get('/warranty-claims', requireRoles(['admin', 'super_admin']), async (req, res) => {
    try {
        const claimsRes = await db_1.db.query(`SELECT wc.*, w.warranty_number, w.duration, w.expiry_date,
 ...
 */
exports.adminRouter.post('/warranty-claims/:claimId/inspect', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
        await db_1.db.transaction(async (client) => {
            ...
            await client.query(`UPDATE warranty_claims 
         SET status = $1, admin_outcome = $2, updated_at = NOW() 
         WHERE id = $3`, [statusValue, outcome, claimId]);
            // Write audit log
            if (admin) {
 ...
 */
exports.adminRouter.get('/trust-scores', requireRoles(['admin', 'super_admin']), async (req, res) => {
    try {
        const scoresRes = await db_1.db.query(`SELECT ts.*, u.full_name as fundi_name, fp.completed_jobs
 ...
 */
exports.adminRouter.post('/badges/award', requireRoles(['admin', 'super_admin']), async (req, res) => {
    const { fundiProfileId, badgeName } = req.body;
    const admin = req.body.adminUser;
    try {
        // Get badge ID
        const badgeRes = await db_1.db.query('SELECT id FROM badges WHERE name = $1', [badgeName]);
        ...
        // Map badge
        await db_1.db.query(`INSERT INTO fundi_badges (fundi_profile_id, badge_id)
       VALUES ($1, $2)
       ON CONFLICT DO NOTHING`, [fundiProfileId, badgeId]);
        if (admin) {
 ...
 */
exports.adminRouter.post('/fundis/level', requireRoles(['admin', 'super_admin']), async (req, res) => {
    ...
    try {
       ...
       SET current_level = EXCLUDED.current_level,
           updated_at = NOW()`, [fundiProfileId, targetLevel]);
        if (admin) {
