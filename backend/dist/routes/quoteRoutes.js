 ...
 */
exports.quoteRouter.post('/request', async (req, res) => {
    const { customerId, professionId, title, description, budgetMin, budgetMax, photosUrls, regionId, districtId, wardId, villageId } = req.body;
    try {
        if (!customerId || !professionId || !title || !description) {
            res.status(400).json({ error: 'Missing required fields for quote request' });
            return;
        }
        const result = await db_1.db.query(`INSERT INTO job_posts (
 ...
 */
exports.quoteRouter.get('/customer/:customerId', async (req, res) => {
    const { customerId } = req.params;
    try {
        const requestsRes = await db_1.db.query(`SELECT jp.*, p.name_en as profession_name_en, p.name_sw as profession_name_sw
       FROM job_posts jp
       JOIN professions p ON jp.profession_id = p.id
       WHERE jp.customer_id = $1
       ORDER BY jp.created_at DESC`, [customerId]);
        const requests = requestsRes.rows;
        // Load bids for each request
        for (const reqObj of requests) {
            const bidsRes = await db_1.db.query(`SELECT jb.*, u.full_name as fundi_name, u.profile_picture_url as fundi_picture,
         ...
         ORDER BY jb.created_at DESC`, [reqObj.id]);
 ...
 */
exports.quoteRouter.get('/fundi/:fundiId', async (req, res) => {
    const { fundiId } = req.params;
    try {
        // 1. Get fundi profession
        const fundiRes = await db_1.db.query('SELECT profession_id FROM fundi_profiles WHERE user_id = $1', [fundiId]);
        ...
        const requestsRes = await db_1.db.query(`SELECT jp.*, u.full_name as customer_name, u.profile_picture_url as customer_picture
       FROM job_posts jp
       JOIN users u ON jp.customer_id = u.id
       WHERE jp.profession_id = $1 AND jp.is_active = true
       ORDER BY jp.created_at DESC`, [profession_id]);
        const requests = requestsRes.rows;
        // Check if the fundi has already bid on each request
        for (const reqObj of requests) {
            const bidRes = await db_1.db.query('SELECT * FROM job_bids WHERE job_post_id = $1 AND fundi_id = $2', [reqObj.id, fundiId]);
 ...
 */
exports.quoteRouter.post('/bid', async (req, res) => {
    const { jobPostId, fundiId, quotationAmount, estimatedDuration, coverLetter, warrantyPeriod, materialsIncluded, completionTime, notes } = req.body;
    try {
        ...
        const checkRes = await db_1.db.query('SELECT id FROM job_bids WHERE job_post_id = $1 AND fundi_id = $2', [jobPostId, fundiId]);
        let result;
        if (checkRes.rowCount !== null && checkRes.rowCount > 0) {
            // Update existing bid
            result = await db_1.db.query(`UPDATE job_bids 
         ...
         WHERE job_post_id = $8 AND fundi_id = $9 RETURNING *`, [
            ...
            // Insert new bid
            result = await db_1.db.query(`INSERT INTO job_bids (
 ...
 */
exports.quoteRouter.patch('/bid/:bidId/accept', async (req, res) => {
    const { bidId } = req.params;
    try {
        // 1. Get bid info
        const bidRes = await db_1.db.query(`SELECT jb.*, jp.customer_id, jp.profession_id, jp.description as job_desc 
       FROM job_bids jb
       ...
       WHERE jb.id = $1`, [bidId]);
        ...
        const booking = await db_1.db.transaction(async (client) => {
            // Mark this bid as accepted
            await client.query(`UPDATE job_bids SET status = 'accepted' WHERE id = $1`, [bidId]);
            // Mark other bids for this post as rejected
            await client.query(`UPDATE job_bids SET status = 'rejected' WHERE job_post_id = $1 AND id != $2`, [bid.job_post_id, bidId]);
            // Close the request post
            await client.query(`UPDATE job_posts SET is_active = false WHERE id = $1`, [bid.job_post_id]);
            // Create booking from quote details
            const bookingRes = await client.query(`INSERT INTO bookings (
