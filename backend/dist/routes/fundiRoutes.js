...
const nidaService_1 = require("../services/nidaService");
 ...
 */
exports.fundiRouter.post('/upload', authMiddleware_1.authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        let mimeType;
        try {
            mimeType = securityService_1.SecurityService.validateMagicBytes(fileBuffer, ext);
        }
        catch (e) {
        ...
        const isClean = await securityService_1.SecurityService.scanFileForViruses(fileBuffer);
        if (!isClean) {
        ...
        const encryptedPath = securityService_1.SecurityService.encrypt(savedPath);
        res.status(201).json({
...
exports.fundiRouter.get('/search', async (req, res) => {
    const { professionId, primaryOnly, regionId, districtId, wardId, minRating, minExperience, maxExperience, skillLevel, // 'Beginner' | 'Intermediate' | 'Professional' | 'Expert' | 'Master'
    minPrice, maxPrice, emergencyService, vetaCertified, topRated, premiumFundi, language, // 'Swahili' | 'English' etc.
    verified, // general verified_badge
    online, latitude, longitude, radiusKm = '15', sortBy, // 'rating' | 'experience' | 'distance' | 'price'
    keyword, warranty } = req.query;
    try {
        let sql = `
      SELECT 
        u.id, 
...
exports.fundiRouter.get('/profile/:userId', async (req, res) => {
    ...
    try {
       ...
       WHERE u.id = $1
       GROUP BY u.id, fp.user_id, r.id, d.id, w.id, v.id`, [userId]);
        if (profileRes.rowCount === 0) {
            res.status(404).json({ error: 'Fundi profile not found' });
            return;
        }
        // 2. Fetch specific professions list
        const professionsRes = await db_1.db.query(`SELECT fp.*, p.name_en, p.name_sw 
       FROM fundi_professions fp 
       JOIN professions p ON fp."professionId" = p.id 
       WHERE fp."fundiId" = $1`, [userId]);
        // 3. Fetch certificates
        const certificatesRes = await db_1.db.query(`SELECT * FROM certificates WHERE fundi_id = $1 ORDER BY created_at DESC`, [userId]);
        ...
        const certificates = certificatesRes.rows.map(cert => {
            if (cert.image_url && cert.image_url.includes(':')) {
                try {
                    cert.image_url = securityService_1.SecurityService.decrypt(cert.image_url);
                }
                catch {
        ...
        const educationRes = await db_1.db.query(`SELECT * FROM education WHERE fundi_id = $1 ORDER BY start_date DESC`, [userId]);
        // 5. Fetch licenses list
        const licensesRes = await db_1.db.query(`SELECT * FROM licenses WHERE fundi_profile_id = $1 ORDER BY created_at DESC`, [userId]);
        const licenses = licensesRes.rows.map(lic => {
            if (lic.credential_url && lic.credential_url.includes(':')) {
                try {
                    lic.credential_url = securityService_1.SecurityService.decrypt(lic.credential_url);
                }
                catch {
                    // Fallback
                }
        ...
        const portfolioRes = await db_1.db.query(`SELECT * FROM portfolio_items WHERE fundi_id = $1 ORDER BY completion_date DESC`, [userId]);
        const portfolio = portfolioRes.rows.map(item => {
            if (item.before_image_url && item.before_image_url.includes(':')) {
                try {
                    item.before_image_url = securityService_1.SecurityService.decrypt(item.before_image_url);
                }
                catch { }
            }
            if (item.after_image_url && item.after_image_url.includes(':')) {
                try {
                    item.after_image_url = securityService_1.SecurityService.decrypt(item.after_image_url);
                }
                catch { }
            }
            if (item.video_url && item.video_url.includes(':')) {
                try {
                    item.video_url = securityService_1.SecurityService.decrypt(item.video_url);
                }
                catch { }
            }
            return item;
 ...
 */
exports.fundiRouter.put('/profile', authMiddleware_1.authenticateJWT, async (req, res) => {
    ...
    const parseResult = profileValidator_1.profileUpdateSchema.safeParse(req.body);
    if (!parseResult.success) {
        res.status(400).json({ error: 'Validation failed', details: parseResult.error.format() });
        return;
    }
    const { bio, experienceYears, startingPrice, languagesSpoken, serviceAreaType, serviceAreaRadius, emergencyServiceEnabled, vacationMode, vacationStart, vacationEnd, workingDays, workingHoursStart, workingHoursEnd, lunchBreakStart, lunchBreakEnd, professions } = parseResult.data;
    try {
         ...
         WHERE user_id = $16`, [
            ...
            if (professions && professions.length > 0) {
                ...
                for (const prof of professions) {
                    await client.query(`INSERT INTO fundi_professions (
 ...
 */
exports.fundiRouter.post('/certificates', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: 'Unauthorized session' });
        return;
    }
    const parseResult = profileValidator_1.certificateSchema.safeParse(req.body);
    if (!parseResult.success) {
        res.status(400).json({ error: 'Validation failed', details: parseResult.error.format() });
        return;
    }
    ...
    try {
        const result = await db_1.db.query(`INSERT INTO certificates (
        fundi_id, name, institution, certificate_number, issue_date, expiry_date, image_url, verification_status
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending_verification') RETURNING *`, [userId, name, institution, certificateNumber, new Date(issueDate), expiryDate ? new Date(expiryDate) : null, imageUrl]);
 ...
 */
exports.fundiRouter.delete('/certificates/:id', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    const { id } = req.params;
    try {
        const result = await db_1.db.query(`DELETE FROM certificates WHERE id = $1 AND fundi_id = $2 RETURNING *`, [id, userId]);
 ...
 */
exports.fundiRouter.post('/education', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: 'Unauthorized session' });
        return;
    }
    const parseResult = profileValidator_1.educationSchema.safeParse(req.body);
    if (!parseResult.success) {
        res.status(400).json({ error: 'Validation failed', details: parseResult.error.format() });
        return;
    }
    ...
    try {
        const result = await db_1.db.query(`INSERT INTO education (
        fundi_id, institution, course, level, start_date, end_date
      ) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [userId, institution, course, level, new Date(startDate), new Date(endDate)]);
 ...
 */
exports.fundiRouter.delete('/education/:id', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    const { id } = req.params;
    try {
        const result = await db_1.db.query(`DELETE FROM education WHERE id = $1 AND fundi_id = $2 RETURNING *`, [id, userId]);
        if (result.rowCount === 0) {
 ...
 */
exports.fundiRouter.post('/licenses', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: 'Unauthorized session' });
        return;
    }
    const parseResult = profileValidator_1.licenseSchema.safeParse(req.body);
    if (!parseResult.success) {
        res.status(400).json({ error: 'Validation failed', details: parseResult.error.format() });
        return;
    }
    ...
    try {
        const result = await db_1.db.query(`INSERT INTO licenses (
        fundi_profile_id, license_number, authority, expiry_date, status, credential_url
      ) VALUES ($1, $2, $3, $4, 'pending_verification', $5) RETURNING *`, [userId, licenseNumber, authority, new Date(expiryDate), credentialUrl]);
 ...
 * @desc Delete license
 */
exports.fundiRouter.delete('/licenses/:id', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    const { id } = req.params;
    try {
        const result = await db_1.db.query(`DELETE FROM licenses WHERE id = $1 AND fundi_profile_id = $2 RETURNING *`, [id, userId]);
        if (result.rowCount === 0) {
 ...
 */
exports.fundiRouter.post('/portfolio', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        res.status(401).json({ error: 'Unauthorized session' });
        return;
    }
    const parseResult = profileValidator_1.portfolioItemSchema.safeParse(req.body);
    if (!parseResult.success) {
        res.status(400).json({ error: 'Validation failed', details: parseResult.error.format() });
        return;
    }
    const { title, description, mediaUrls, videoUrl, beforeImageUrl, afterImageUrl, completionDate, clientApproved, serviceCategory, location, clientReviewId } = parseResult.data;
    try {
        const result = await db_1.db.query(`INSERT INTO portfolio_items (
 ...
 */
exports.fundiRouter.delete('/portfolio/:id', authMiddleware_1.authenticateJWT, async (req, res) => {
    const userId = req.user?.id;
    const { id } = req.params;
    try {
        const result = await db_1.db.query(`DELETE FROM portfolio_items WHERE id = $1 AND fundi_id = $2 RETURNING *`, [id, userId]);
        if (result.rowCount === 0) {
 ...
 */
exports.fundiRouter.post('/subscribe', authMiddleware_1.authenticateJWT, async (req, res) => {
 ...
 */
exports.fundiRouter.post('/nida/verify', async (req, res) => {
    ...
    try {
        const verification = await nidaService_1.NIDAService.verifyNIDA(nida, fundiProfileId);
