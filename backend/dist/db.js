...
function mockDatabaseQuery(text, params) {
    ...
    if (queryStr.includes('select id from chats')) {
        ...
        const chat = mockChats.find(c => (c.participant_one_id === p1 && c.participant_two_id === p2) ||
    ...
    if (queryStr.includes('wallets')) {
        ...
        if (queryStr.includes('update')) {
            if (queryStr.includes('balance - $1') || queryStr.includes('balance = balance -') || queryStr.includes('escrow_held = escrow_held +')) {
                const amt = params ? parseFloat(params[0]) : 0;
                wallet.balance -= amt;
...
const mockPoolClient = {
    query: async (text, params) => mockDatabaseQuery(text, params),
    release: () => { }
};
exports.db = {
    /**
