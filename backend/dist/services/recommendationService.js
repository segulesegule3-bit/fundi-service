...
class RecommendationService {
     ...
     */
    static async recommendFundis(req) {
    ...
    `;
        const result = await db_1.db.query(query, [professionId]);
        const fundis = [];
        ...
        for (const row of result.rows) {
            ...
            const aiScore = parseFloat((primaryScore + distanceScore + experienceScore + skillScore + ratingScore + responseScore + availabilityScore + priceScore + repeatCustomerScore).toFixed(2));
            fundis.push({
                userId: row.user_id,
