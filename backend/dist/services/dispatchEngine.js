...
class DispatchEngine {
    static activeDispatches = new Map();
    /**
     * Dispatches a booking job request in incremental radius waves
     */
    static async dispatchBooking(bookingId, waveAttempt = 1) {
        const radiusSteps = [1, 3, 5, 10]; // Radii: 1km, 3km, 5km, 10km
        if (waveAttempt > radiusSteps.length) {
            logger_1.default.info(`[Dispatch Engine] No available fundi accepted booking #${bookingId} after expanding to 10km.`);
        ...
        logger_1.default.info(`[Dispatch Engine] Initiating wave #${waveAttempt} for booking #${bookingId} within ${currentRadius}km`);
        ...
        const nearbyFundis = await gpsService_1.GPSService.findNearbyFundis(clientLat, clientLng, currentRadius);
        if (nearbyFundis.length === 0) {
            // No fundis found: immediately jump to next wave
            logger_1.default.info(`[Dispatch Engine] No online fundis within ${currentRadius}km. Expanding radius...`);
        ...
        const scoredFundis = nearbyFundis.map(fundi => {
            const matchScore = aiService_1.AIService.calculateMatchScore(fundi.fundiId, clientLat, clientLng, parseFloat(fundi.rating || '4.0'));
        ...
        for (const candidate of candidates) {
            ...
            await prisma_1.default.jobDispatchQueue.create({
                data: {
                    jobId: bookingId,
            ...
            dispatchSocket_1.DispatchSocket.sendToUser(candidate.fundiId, 'job:new', {
                bookingId,
        ...
        const timeout = setTimeout(async () => {
            const refreshedBooking = await prisma_1.default.booking.findUnique({
                where: { id: bookingId },
            });
            // If still pending, expand radius to next wave
            if (refreshedBooking && refreshedBooking.status === client_1.BookingStatus.PENDING) {
                logger_1.default.info(`[Dispatch Engine] Wave #${waveAttempt} timeout for booking #${bookingId}. Expanding radius...`);
                await DispatchEngine.dispatchBooking(bookingId, waveAttempt + 1);
            }
        }, 30000);
     ...
     */
    static clearDispatch(bookingId) {
