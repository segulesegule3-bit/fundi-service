...
class AuthController {
    ...
    static async registerCustomer(req, res) {
        try {
            const user = await authService_1.AuthService.registerCustomer(req.body);
            res.status(201).json({
                success: true,
     ...
     */
    static async registerFundi(req, res) {
        try {
            const user = await authService_1.AuthService.registerFundi(req.body);
            res.status(201).json({
                success: true,
    ...
    static async verifyOTP(req, res) {
        ...
        try {
            ...
            const verified = await otpService_1.OTPService.verifyOTP(user.id, code, purpose || 'registration');
            if (!verified) {
                res.status(400).json({
                    success: false,
                    message: 'OTP verification failed',
    ...
    static async resendOTP(req, res) {
        ...
        try {
            ...
            const code = await otpService_1.OTPService.createOTP(user.id, purpose || 'registration');
    ...
    static async login(req, res) {
        ...
        const lockout = rateLimiter_1.BruteForceProtector.isLockedOut(identifier);
        if (lockout.locked) {
            res.status(401).json({
                success: false,
        ...
        try {
            const result = await authService_1.AuthService.login(req.body, { ip, userAgent, deviceName });
            if (!result) {
                const attempts = rateLimiter_1.BruteForceProtector.recordFailure(identifier);
            ...
            rateLimiter_1.BruteForceProtector.recordSuccess(identifier);
            res.status(200).json({
                success: true,
    ...
    static async logout(req, res) {
        ...
        const success = await authService_1.AuthService.logout(sessionId);
        if (!success) {
            res.status(500).json({
                success: false,
                message: 'Logout failed',
    ...
    static async logoutAll(req, res) {
        ...
        const count = await authService_1.AuthService.logoutAll(userId);
        res.status(200).json({
            success: true,
    ...
    static async refreshToken(req, res) {
        ...
        const result = await authService_1.AuthService.refreshSession(refreshToken);
        if (!result) {
            res.status(401).json({
                success: false,
                message: 'Token rotation failed',
    ...
    static async getSessions(req, res) {
        const userId = req.user?.id;
        try {
            const sessions = await prisma_1.default.loginSession.findMany({
                where: { userId },
                select: {
                    id: true,
                    ipAddress: true,
    ...
    static async deleteSession(req, res) {
        ...
        const userId = req.user?.id;
        try {
            const session = await prisma_1.default.loginSession.findUnique({
                where: { id },
            });
    ...
    static async forgotPassword(req, res) {
        ...
        const identifier = email || phoneNumber;
        try {
            const success = await authService_1.AuthService.requestPasswordReset(identifier);
            if (!success) {
                res.status(404).json({
                    success: false,
    ...
    static async resetPassword(req, res) {
        ...
        try {
            const success = await authService_1.AuthService.resetPassword(phoneNumber, code, newPassword);
            if (!success) {
                res.status(400).json({
                    success: false,
