    ...
    static init(io) {
        io.on('connection', (socket) => {
            ...
            socket.on('authenticate', async (data) => {
                ...
                await db_1.db.query(`UPDATE fundi_profiles SET online_status = true, last_seen = NOW() WHERE user_id = $1`, [decoded.id]);
            ...
            socket.on('send_message', async (data) => {
                ...
                try {
                    ...
                    const messageRes = await db_1.db.query(`INSERT INTO messages (chat_id, sender_id, text, attachment_url, attachment_type, latitude, longitude)
             VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [chatId, senderId, text || null, attachmentUrl || null, attachmentType || null, latitude || null, longitude || null]);
                    const savedMessage = messageRes.rows[0];
                    // Update last message in chat summary
                    await db_1.db.query(`UPDATE chats SET last_message = $1, last_message_at = NOW() WHERE id = $2`, [text || `[Attachment: ${attachmentType}]`, chatId]);
            ...
            socket.on('mark_read', async (data) => {
                ...
                if (currentUserId !== userId)
                    return;
                try {
                    await db_1.db.query(`UPDATE messages SET is_read = true, read_at = NOW() 
             WHERE id = ANY($1) AND sender_id != $2`, [messageIds, userId]);
           ...
           WHERE user_id = $2`, [pointGeo, currentUserId]);
            ...
            socket.on('disconnect', async () => {
                ...
                if (currentUserId) {
                    ...
                    // Update online status in database
                    await db_1.db.query(`UPDATE fundi_profiles SET online_status = false, last_seen = NOW() WHERE user_id = $1`, [currentUserId]);
