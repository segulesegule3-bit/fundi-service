...
describe('Quotation & Negotiation Pricing Workflow', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            customerUser = await prisma_1.default.user.create({
                data: {
        ...
        try {
            ...
            await db_1.db.query('INSERT INTO wallets (user_id, balance, escrow_held) VALUES ($1, 200000.00, 0.00)', [customerId]);
        }
        catch {
            // Mock db handles queries automatically
        }
        const custTokens = authService_1.AuthService.generateTokens(customerUser, 'session_cust');
        customerToken = custTokens.accessToken;
        let fundiUser = null;
        try {
            // 3. Create Fundi User & Profile
            fundiUser = await prisma_1.default.user.create({
                data: {
                    phoneNumber: '+255999000' + Math.floor(100 + Math.random() * 900),
                    passwordHash: 'dummy_hash',
                    role: 'FUNDI',
        ...
        try {
            await prisma_1.default.fundiProfile.create({
                data: {
                    userId: fundiId,
        ...
        try {
            ...
            await prisma_1.default.fundiProfession.create({
                data: {
                    fundiId,
                    professionId,
                    startingPrice: 15000.00,
        ...
        const fTokens = authService_1.AuthService.generateTokens(fundiUser, 'session_fundi');
    ...
    afterAll(async () => {
        try {
            ...
            if (bookingId) {
                await db_1.db.query('DELETE FROM payments WHERE booking_id = $1', [bookingId]);
                await db_1.db.query('DELETE FROM bookings WHERE id = $1', [bookingId]);
            }
            await db_1.db.query('DELETE FROM wallets WHERE user_id = $1', [customerId]);
            await db_1.db.query('DELETE FROM fundi_professions WHERE "fundiId" = $1', [fundiId]);
            await db_1.db.query('DELETE FROM fundi_profiles WHERE user_id = $1', [fundiId]);
            await db_1.db.query('DELETE FROM users WHERE id IN ($1, $2)', [customerId, fundiId]);
            await db_1.db.query('DELETE FROM professions WHERE id = $1', [professionId]);
        }
        catch {
    ...
    describe('Booking Request with Quotation-Based fields', () => {
        it('should create booking and populate AI estimation, starting price, and inspection fee', async () => {
            ...
            const checkRes = await db_1.db.query('SELECT * FROM bookings WHERE id = $1', [bookingId]);
    ...
    describe('Acceptance, locking price, and holding escrow', () => {
        it('should accept quotation and lock funds in escrow', async () => {
            ...
            const walletRes = await db_1.db.query('SELECT balance, escrow_held FROM wallets WHERE user_id = $1', [customerId]);
