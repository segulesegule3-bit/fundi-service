...
export class SMSService {
  private static gateway: string = process.env.SMS_GATEWAY || 'mock';
   ...
   */
  public static async sendSMS(to: string, text: string): Promise<SMSSendResult> {
  ...
  private static async sendBeemSMS(to: string, text: string): Promise<SMSSendResult> {
  ...
  private static async sendTwilioSMS(to: string, text: string): Promise<SMSSendResult> {
  ...
  private static async sendAfricasTalkingSMS(to: string, text: string): Promise<SMSSendResult> {
    // Mock implementation for Africa's Talking API
    return { success: true, messageId: `africastalking-${Date.now()}` };
  }

  private static async sendMockSMS(to: string, text: string): Promise<SMSSendResult> {
