...
export class TrustSafetyService {
   ...
   */
  public static async recalculateTrustScore(fundiProfileId: string): Promise<number> {
    try {
      // 1. Load Fundi profile stats
      const fundiRes = await db.query(
        `SELECT fp.id, fp.verified_badge, fp.completed_jobs,
      ...
      if (fundiRes.rowCount === 0) return 100;
      ...
      const certRes = await db.query(
        `SELECT COUNT(*) as count FROM certificates WHERE fundi_profile_id = $1 AND status = 'active'`,
        [fundiProfileId]
      );
      ...
      const claimsRes = await db.query(
        `SELECT COUNT(*) as count 
         ...
         WHERE b.fundi_id = (SELECT user_id FROM fundi_profiles WHERE id = $1) AND wc.status = 'ACCEPTED'`,
        [fundiProfileId]
      );
      ...
      if (verifiedIdentity) score += 10;
      if (verifiedCertificates) score += 10;
      ...
      // 3. Update database
      await db.query(
        `INSERT INTO trust_scores (fundi_profile_id, score, verified_identity, verified_certificates, jobs_completed, avg_rating, response_time_mins)
         ...
         SET score = EXCLUDED.score,
             ...
             updated_at = NOW()`,
        [fundiProfileId, finalScore, verifiedIdentity, verifiedCertificates, profile.completed_jobs || 0, ratingVal]
      );
   ...
   */
  private static async updateProgressionLevel(fundiProfileId: string, trustScore: number, jobsCount: number): Promise<string> {
    let level = 'BRONZE';
    ...
    await db.query(
      `INSERT INTO fundi_levels (fundi_profile_id, current_level, points)
       ...
       SET current_level = EXCLUDED.current_level,
           ...
           updated_at = NOW()`,
      [fundiProfileId, level, jobsCount * 10]
    );
