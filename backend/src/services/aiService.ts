...
export class AIService {
   ...
   */
  public static calculateMatchScore(fundiId: string, clientLat: number, clientLng: number, rating: number): number {
    const coords = GPSService.getLocation(fundiId);
    let distance = 5.0; // default assumptions if no live coordinates exist

    if (coords) {
      distance = GPSService.calculateDistance(clientLat, clientLng, coords.latitude, coords.longitude);
    }
   ...
   */
  public static estimatePrice(category: string, urgency: string): { minPrice: number; maxPrice: number; recommendedPrice: number } {
    let baseMin = 15000;
    let baseMax = 35000;
   ...
   */
  public static async checkFraudScore(userId: string, flags: { cancellationCount: number; isFakeGps: boolean }): Promise<number> {
    let score = 0;

    if (flags.isFakeGps) score += 60;
    if (flags.cancellationCount > 3) score += 30;

    if (score >= 80) {
      logger.warn(`[AI Fraud Alert] User ${userId} exceeded safety thresholds. Suspending account automatically...`);
   ...
   */
  public static getDemandInsights(region: string): any {
    return {
      region,
   ...
   */
  public static async chatbotReply(message: string): Promise<string> {
   ...
   */
  public static getFairMarketPrice(category: string): { fairPrice: number; currency: string } {
    let fairPrice = 25000;
    switch (category.toLowerCase()) {
      case 'electrical':
      case 'electrician':
   ...
   */
  public static getPopularServices(): string[] {
   ...
   */
  public static getHighDemandLocations(): { lat: number; lng: number; name: string; demandLevel: string }[] {
    return [
   ...
   */
  public static recommendWarranty(category: string): string {
    switch (category.toLowerCase()) {
      case 'electrical':
      case 'electrician':
        return '90 Days';
   ...
   */
  public static async detectAbnormalClaimPatterns(fundiId: string): Promise<boolean> {
    try {
      const claimsRes = await db.query(
        `SELECT COUNT(*) as count 
         ...
         WHERE b.fundi_id = $1 AND wc.status = 'ACCEPTED'`,
        [fundiId]
      );
   ...
   */
  public static async identifyHighRiskFundis(): Promise<any[]> {
    try {
      const risksRes = await db.query(
        `SELECT ts.*, u.full_name as fundi_name
