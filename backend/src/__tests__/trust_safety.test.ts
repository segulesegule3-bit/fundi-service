...
describe('Trust, Safety & Warranty Integrations', () => {
  ...
  describe('Trust Score Calculation Calculations', () => {
    it('should correctly calculate trust score with active badges and jobs', async () => {
      ...
      // Recalculate
      const score = await TrustSafetyService.recalculateTrustScore('fundi-profile-123');
      ...
      expect(db.query).toHaveBeenCalledTimes(5); // SELECT profile, SELECT certs, SELECT claims, INSERT score, INSERT level
    });
    ...
    it('should deduct points for accepted warranty claims', async () => {
      ...
      // Recalculate
      const score = await TrustSafetyService.recalculateTrustScore('fundi-profile-456');
  ...
  describe('Duration Parsing Rules', () => {
    it('should map warranty durations to days correctly', () => {
      const getWarrantyDays = (period: string): number => {
