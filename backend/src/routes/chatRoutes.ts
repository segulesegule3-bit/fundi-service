 ...
 */
chatRouter.get('/user/:userId', async (req: Request, res: Response): Promise<void> => {
 ...
 */
chatRouter.get('/:chatId/messages', async (req: Request, res: Response): Promise<void> => {
  const { chatId } = req.params;
  const { limit = '50', offset = '0' } = req.query;

  try {
    const messagesRes = await db.query(
      `SELECT m.id, m.sender_id, m.text, m.attachment_url, m.attachment_type, 
 ...
 */
chatRouter.post('/create', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const chatRes = await db.query(
      `INSERT INTO chats (participant_one_id, participant_two_id)
       VALUES ($1, $2)
