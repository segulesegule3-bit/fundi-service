...
appRouter.get('/version', async (req: Request, res: Response) => {
  try {
    const latestApk = await db.query(
      `SELECT id, version_code, release_notes, type, download_url, force_update, created_at 
       FROM app_releases WHERE type = 'apk' AND is_published = true 
       ORDER BY created_at DESC LIMIT 1`
    );

    const latestAab = await db.query(
      `SELECT id, version_code, release_notes, type, download_url, force_update, created_at 
       FROM app_releases WHERE type = 'aab' AND is_published = true 
       ORDER BY created_at DESC LIMIT 1`
    );

    const latestIos = await db.query(
      `SELECT id, version_code, release_notes, type, download_url, force_update, created_at 
 ...
 */
appRouter.get('/download/apk', async (req: Request, res: Response) => {
  try {
    const latestApkRes = await db.query(
      `SELECT id, download_url FROM app_releases 
    ...
    await db.query(
      `INSERT INTO app_download_analytics (release_id, platform, ip_address, device_type, os_version, country)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [latestApk.id, 'apk', ipAddress, device, os, 'Tanzania']
    );
 ...
 */
appRouter.get('/redirect/play-store', async (req: Request, res: Response) => {
  try {
    const latestApkRes = await db.query(
      `SELECT id FROM app_releases WHERE type = 'apk' AND is_published = true ORDER BY created_at DESC LIMIT 1`
    );
    ...
    await db.query(
      `INSERT INTO app_download_analytics (release_id, platform, ip_address, device_type, os_version, country)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [releaseId, 'play_store', ipAddress, device, os, 'Tanzania']
    );
 ...
 */
appRouter.get('/redirect/app-store', async (req: Request, res: Response) => {
  try {
    const latestIosRes = await db.query(
      `SELECT id, download_url FROM app_releases WHERE type = 'ios' AND is_published = true ORDER BY created_at DESC LIMIT 1`
    );
    ...
    await db.query(
      `INSERT INTO app_download_analytics (release_id, platform, ip_address, device_type, os_version, country)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [releaseId, 'app_store', ipAddress, device, os, 'Tanzania']
    );
 ...
 */
appRouter.get('/latest', async (req: Request, res: Response) => {
  try {
    const latestApkRes = await db.query(
      `SELECT version_code, release_notes, download_url, force_update 
       FROM app_releases WHERE type = 'apk' AND is_published = true 
       ORDER BY created_at DESC LIMIT 1`
    );
