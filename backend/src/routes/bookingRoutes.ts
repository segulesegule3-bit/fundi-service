...
import { validateBody, bookingCreateSchema } from '../middlewares/validationMiddleware';
 ...
 */
bookingRouter.post('/create', authenticateJWT, validateBody(bookingCreateSchema), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const fprofRes = await db.query(
      `SELECT starting_price, inspection_fee 
       FROM fundi_professions 
       WHERE "fundiId" = $1 AND "professionId" = $2`,
      [fundiId, professionId]
    );
    ...
    if (fprofRes.rowCount !== null && fprofRes.rowCount > 0) {
      ...
      const fprofProfile = await db.query(
        'SELECT starting_price, inspection_fee FROM fundi_profiles WHERE user_id = $1',
        [fundiId]
      );
    ...
    const profRes = await db.query('SELECT name_en FROM professions WHERE id = $1', [professionId]);
 ...
 */
bookingRouter.patch('/:bookingId/status', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    await db.transaction(async (client) => {
      ...
      if (booking.payment_option !== 'offline') {
        if (status === 'accepted' || status === 'price_confirmed') {
          ...
          await WalletService.holdEscrow(booking.customer_id, parseFloat(booking.service_price), bookingId);
        } 
        else if (status === 'rejected' || status === 'cancelled') {
          // If funds were already held, refund them
          if (previousStatus !== 'pending') {
            await WalletService.refundEscrow(bookingId);
          }
        } 
        else if (status === 'completed') {
          await WalletService.releaseEscrow(bookingId);
        }
      }

      if (status === 'completed') {
        // Update Fundi profile completed jobs count
        const fpUpdate = await client.query(
          `UPDATE fundi_profiles SET completed_jobs = completed_jobs + 1 WHERE user_id = $1 RETURNING id`,
          [booking.fundi_id]
        );
        ...
        if (warrantyPeriod !== 'No Warranty' && warrantyPeriod !== '') {
          let durationDays = 30;
          if (warrantyPeriod.includes('7')) durationDays = 7;
          else if (warrantyPeriod.includes('14')) durationDays = 14;
          else if (warrantyPeriod.includes('30')) durationDays = 30;
          else if (warrantyPeriod.includes('60')) durationDays = 60;
          else if (warrantyPeriod.includes('90')) durationDays = 90;
          else if (warrantyPeriod.includes('6 Months') || warrantyPeriod.includes('6 months')) durationDays = 180;
          else if (warrantyPeriod.includes('1 Year') || warrantyPeriod.includes('1 year')) durationDays = 365;
          ...
          await client.query(
            `INSERT INTO warranties (booking_id, warranty_number, duration, start_date, expiry_date, qr_code_data)
             VALUES ($1, $2, $3, $4, $5, $6)
             ON CONFLICT (booking_id) DO NOTHING`,
            [bookingId, warrantyNumber, warrantyPeriod, startDate, expiryDate, qrCodeData]
          );
        }
 ...
 */
bookingRouter.post('/:bookingId/review', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { bookingId } = req.params;
  ...
  try {
    ...
    const bookingRes = await db.query(
      'SELECT fundi_id, status FROM bookings WHERE id = $1',
      [bookingId]
    );
    ...
    const recentReviews = await db.query(
      `SELECT count(*) FROM reviews 
       WHERE customer_id = $1 AND fundi_id = $2 AND created_at > NOW() - INTERVAL '10 minutes'`,
      [customerId, booking.fundi_id]
    );
    ...
    await db.transaction(async (client) => {
      ...
      const averageRes = await client.query(
        `SELECT AVG(rating)::decimal(3,2) as avg_rating, COUNT(*) as count 
         FROM reviews WHERE fundi_id = $1 AND is_fake = false`,
        [booking.fundi_id]
      );
      ...
      await client.query(
        `UPDATE fundi_profiles 
         ...
         WHERE user_id = $3`,
        [avg_rating || 0.00, count || 0, booking.fundi_id]
      );
 ...
 */
bookingRouter.post('/:bookingId/quote', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { bookingId } = req.params;
  ...
  try {
    ...
    await db.query(
      `UPDATE bookings 
       ...
       WHERE id = $6`,
      [price, arrivalTime, completionTime, notes || '', warrantyPeriod || 'No Warranty', bookingId]
    );
    ...
    try {
      ...
      let chatRes = await db.query('SELECT id FROM chats WHERE participant_one_id = $1 AND participant_two_id = $2', [p1, p2]);
      let chatId;
      if (chatRes.rowCount !== null && chatRes.rowCount > 0) {
        chatId = chatRes.rows[0].id;
      } else {
        const newChat = await db.query(
          'INSERT INTO chats (participant_one_id, participant_two_id, last_message, last_message_at) VALUES ($1, $2, $3, NOW()) RETURNING id',
          [p1, p2, 'Negotiation started']
        );
      ...
      await db.query('INSERT INTO messages (chat_id, sender_id, text) VALUES ($1, $2, $3)', [chatId, booking.fundi_id, msgText]);
 ...
 */
bookingRouter.post('/:bookingId/negotiate', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { bookingId } = req.params;
  ...
  try {
    ...
    await db.query(
      `UPDATE bookings 
       SET negotiation_status = $1, last_negotiated_by = $2, last_negotiation_price = $3,
           service_price = $3
       WHERE id = $4`,
      [negotiationStatus, counterBy, counterPrice, bookingId]
    );
    ...
    try {
      ...
      await db.query('INSERT INTO messages (chat_id, sender_id, text) VALUES ($1, $2, $3)', [chatId, senderId, msgText]);
      await db.query('UPDATE chats SET last_message = $1, last_message_at = NOW() WHERE id = $2', [msgText, chatId]);
    } catch (chatErr) {
 ...
 */
bookingRouter.post('/:bookingId/accept-quote', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { bookingId } = req.params;
  ...
  try {
    ...
    await db.transaction(async (client) => {
      ...
      await client.query(
        `UPDATE bookings 
         ...
         WHERE id = $2`,
        [agreedPrice, bookingId]
      );

      // 2. Lock money in Escrow if payment is online
      if (booking.payment_option !== 'offline') {
        await WalletService.holdEscrow(booking.customer_id, agreedPrice, bookingId);
      }
    });
    ...
    try {
      ...
      if (chatRes.rowCount !== null && chatRes.rowCount > 0) {
        ...
        await db.query('INSERT INTO messages (chat_id, sender_id, text) VALUES ($1, $2, $3)', [chatId, booking.customer_id, msgText]);
        await db.query('UPDATE chats SET last_message = $1, last_message_at = NOW() WHERE id = $2', [msgText, chatId]);
      }
    } catch (chatErr) {
 ...
 */
bookingRouter.post('/:bookingId/reject-quote', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { bookingId } = req.params;
