 ...
 */
warrantyRouter.post('/claims', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const warrantyRes = await db.query(
      'SELECT * FROM warranties WHERE booking_id = $1',
      [bookingId]
    );
    ...
    const claimRes = await db.query(
      `INSERT INTO warranty_claims (warranty_id, description, status, photos_urls)
       VALUES ($1, $2, 'PENDING', $3) RETURNING *`,
      [warranty.id, description, photosUrls || '{}']
    );

    const claim = claimRes.rows[0];

    // Send notifications to Fundi
    const bookingRes = await db.query('SELECT fundi_id, service_price FROM bookings WHERE id = $1', [bookingId]);
    if (bookingRes.rowCount !== null && bookingRes.rowCount > 0) {
      const fundiId = bookingRes.rows[0].fundi_id;
      await db.query(
        `INSERT INTO notifications (user_id, title, body, type)
         VALUES ($1, 'Dai la Udhamini (Warranty Claim)', 'Mteja amewasilisha dai la udhamini kwa kazi uliyokamilisha.', 'in_app')`,
        [fundiId]
      );
    }
 ...
 */
warrantyRouter.get('/:id/certificate', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;

  try {
    const warrantyRes = await db.query(
      `SELECT w.*, b.description as booking_description, 
 ...
 */
warrantyRouter.get('/:id/download-pdf', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const { id } = req.params;
 ...
 */
warrantyRouter.patch('/claims/:claimId/status', authenticateJWT, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Update status
    await db.query(
      `UPDATE warranty_claims SET status = $1, updated_at = NOW() WHERE id = $2`,
      [status, claimId]
    );
